...
func TestLRUMaxAge(t *testing.T) {
	c := New(WithAge[string, string](86400))
...
func TestLRUpdateOnGet(t *testing.T) {
	c := New(
		WithAge[string, string](86400),
		WithUpdateAgeOnGet[string, string](),
	)
...
func TestMaxSize(t *testing.T) {
	c := New(WithSize[string, string](2))
	// Add one expired entry
	c.Set("foo", "bar")
	_, ok := c.Get("foo")
	assert.True(t, ok)
...
func TestExist(t *testing.T) {
	...
	c.SetWithExpire(3, 3, tenSecBefore)
	assert.False(t, c.Exist(3))
}
...
func TestEvict(t *testing.T) {
	...
	c := New(WithEvict(evict), WithSize[int, int](1))
	c.Set(1, 2)
	c.Set(2, 3)
...
func TestStale(t *testing.T) {
	c := New(WithAge[int, int](1), WithStale[int, int](true))
	now := time.Now().Unix()
...
func TestCloneTo(t *testing.T) {
	o := New(WithSize[string, int](10))
	o.Set("1", 1)
	o.Set("2", 2)

	n := New(WithSize[string, int](2))
	n.Set("3", 3)
	n.Set("4", 4)
